...
Alternatively, you may place raw CSVs in `data/raw/` and update paths in `config/config.yaml`.
...
```bash
python -m src.pipeline.adaptive_pipeline --config config/config.yaml --sample-size 0.1
```
